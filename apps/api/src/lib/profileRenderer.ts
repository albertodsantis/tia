...
export function generateEfiLinkHtml(params: {
  ...
  const { name, handle, tagline, avatar, socialProfiles, efiProfile, accentColor, forceDark = false, publicUrl = '', posthogKey = '', posthogHost = 'https://us.i.posthog.com' } = params;
...
<head>
  ...
  ${posthogKey ? `<script>
    ...
    posthog.init('${escapeHtml(posthogKey)}', { api_host: '${escapeHtml(posthogHost)}', capture_pageview: true, autocapture: true, disable_session_recording: true });
    posthog.capture('efilink_view', { handle: '${escapeHtml(handle)}' });
  </script>` : ''}
</head>
<body>
  ${crtOverlay}
